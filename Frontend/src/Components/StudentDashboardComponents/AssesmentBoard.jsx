      ...
      <section className="flex flex-col gap-8 md:flex-row lg:flex-row">
        <div className="w-full flex flex-col gap-6">
          <div className="bg-[#FF4081]/10 flex p-5 rounded-md gap-3 items-end">
            ...
            <p className='text-white font-["geist"]text-sms'>
              {assesment.submissionDeadline}
            </p>
          </div>
          <div className="flex flex-col gap-3">
            <p className='text-white font-["geist" font-bold text-lg'>
              Requirements
            </p>
          ...
          <div className="flex flex-col gap-3">
            <p className='font-semibold text-white font-["geist"] text-sm'>
              CodePen Asset Link
            </p>
            <form className="flex flex-col md:flex-row lg:flex-row">
              <input
                type="text"
                placeholder="https://codepen.io/alex-student/pen/XWMyYv"
                className='h-12 px-4 outline-none text-[#94A3B8] font-["Geist-Mono"] text-sm border-none'
              />

              <button className="h-12 px-6 bg-[#2979FF] text-white">
                Verify & Submit
              </button>
            </form>
          </div>
        ...
        <div className="bg-[#161B26] flex flex-col gap-5 p-6 rounded-mds">
          ...
          {assesment.gradingRubrics.map((gr) => (
            <p className='text-white font-["geist"] text-sm'>{gr}</p>
          ))}
        </div>
      </section>
    </section>
